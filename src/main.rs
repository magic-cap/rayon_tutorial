...
use std::fs::File;
use std::io::prelude::*;
use std::io::{BufReader, SeekFrom}; // , Cursor};
...
fn main() -> Result<(), MagicCapError> {
    ...
    print_hello_world();
    let blocksize = 4096;
    let input_file = File::open("very_large_file")?;
    ...
    let mut index_hash: Vec<(usize, [u8; 32])> = bri
        .enumerate()
        .map(|(block_from_zero, (plaintext_block, bytes_read))| {
            (
                block_from_zero,
                (plaintext_block, bytes_read),
                Arc::clone(&outer_file),
            )
        })
        .par_bridge()
        .map(
            |(block_from_zero, (mut plaintext_block, bytes_read), inner_file)| {
                // closure can use bindings from outer scope
                let offset = blocksize * (block_from_zero + 1);
                let mut key = key_from_bytes(key_bytes);
                key.try_seek(offset)
                    .expect("this only fails if we encrypt truly massive files, what?");
                encryptor(&mut key, &mut plaintext_block);
                let crypt_block = plaintext_block; // it's been encrypted in-place
                // println!(
                //     "{bytes_read} {block_from_zero} real actual block number is {}",
                //     block_from_zero + 1
                // );
